  ...
  if (require.main === requiringModule) {
    ...
    function runFuncWithArgs(func, unnamedArgs) {
      ...
      if (Object.keys(namedArgs).length > 0 && unnamedArgs.length > 0) {
        console.error(
          "You cannot specify both named and unnamed arguments to the function"
        );
        process.exit(1);
    ...
    if (requiringModule.exports instanceof Function) {
      ...
      if (targetProperty in requiringModule.exports) {
        ...
        if (requiringModule.exports[targetProperty] instanceof Function) {
          runFuncWithArgs(
            requiringModule.exports[targetProperty].bind(
              requiringModule.exports
            ),
            argv._.slice(1)
          );
        } else {
          // if the target isn't a function, we simply print it
    ...
    if (!targetPropertyFound) {
      ...
      if (validProperties.length === 0) {
        printError(
          "The module you're trying to make runnable doesn't export anything."
        );
      } else {
        // let's give an example of how this should be used
        ...
        var egProperty =
          validFunctionProperties.length > 0
            ? validFunctionProperties[0]
            : validProperties[0];
        ...
        var egPropertyAsArg = egPropertyHasSpace
          ? '"' + egProperty + '"'
          : egProperty;
